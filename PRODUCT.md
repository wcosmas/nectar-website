...
The client name **Allied Health Professionals Council (Uganda)** and the technical detail of that system are cleared for public use. **Buganda Royal Institute** and the ACARIS detail above are likewise cleared.

Both crests are on the client wall, each taken from the institution's own site (`bribte.ac.ug`, `ahpc.go.ug`). Buganda Royal Institute's full registered name is **Buganda Royal Institute of Business and Technical Education (BRIBTE)**; the site uses the short form the institute itself leads with. Written permission to reproduce either crest is **not on file** — worth obtaining before this is treated as settled.
